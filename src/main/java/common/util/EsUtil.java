...
//				.startObject("writing_type").field("type", "keyword").endObject()
	 ...
	 * 将实体类的属性封装到Map中
	 * @param writing
	 * @param categoryName 分类的service
	 * @return
	 */
	public static <T extends Writing> JSONObject getJsonObject(T writing, String categoryName) throws Exception {
		JSONObject jsonMap = new JSONObject();
//		jsonMap.put("writing_id", writing.getId());
		// 因为文章和评论的id可能会重复，且放在同一个索引下
		// 所以这里添加两个参数 作品类型 及其id
//		jsonMap.put("writing_type", getWritingTypeStr(writing));

		jsonMap.put("athorId", writing.getAuthorId());
		jsonMap.put("title", writing.getTitle());
		jsonMap.put("content", writing.getContent());

		jsonMap.put("category_id", writing.getCategory());
		jsonMap.put("category_name", categoryName);
	 ...
	 */
	public static String getBulkRespLog(BulkResponse bulkResponse, Logger logger) {
